locals {
  ...
  default_redis_config = {
    ...
    rdb_storage_connection_string = local.data_persistence_enabled == "true" ? join("", azurerm_storage_account.redis_storage.*.primary_blob_connection_string) : null
    rdb_backup_frequency          = local.data_persistence_enabled == "true" ? var.data_persistence_frequency_in_minutes : null
    rdb_backup_max_snapshot_count = local.data_persistence_enabled == "true" ? var.data_persistence_max_snapshot_count : null
  }
